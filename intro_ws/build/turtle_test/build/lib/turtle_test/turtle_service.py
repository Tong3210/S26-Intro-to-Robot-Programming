...
class TurtleServer(Node):
    def __init__(self):
        ...
        self._action_server = ActionServer(self, MakeSquare, 'make_square', execute_callback=self.execute_callback, goal_callback=self.goal_callback, cancel_callback=self.cancel_callback)
    ...
    async def execute_callback(self, goal_handle):
        ...
        for i in range(4):
            ...
            feedback_msg.current_pose.position = [float(x) for x in self.turtle.position()]
            goal_handle.publish_feedback(feedback_msg)
            time.sleep(1)
        goal_handle.succeed()
        result = MakeSquare.Result()
        result.final_pose.heading = float(self.turtle.heading())
        result.final_pose.position = [float(x) for x in self.turtle.position()]
